    ...
    <v-card class="right-fixed-panel pl-3 v-scrolling-div" :width="$vuetify.breakpoint.mdAndUp ? '70vw' : ''">
      <v-container class="px-2 d-flex flex-column">
        ...
        <v-row class="align-center">
            <v-btn icon class="my-auto ml-3" @click="tab == 'tab-payments' ? (tab = 'tab-analysis') : (tab = 'tab-details')">
                <v-icon>
                    mdi-arrow-left
                </v-icon> 
            </v-btn>
            <v-col cols="4" class="d-flex flex-column pl-0 my-auto">
                <h3>{{DialogHeader}}</h3>
                <p class="mb-0">Please enter the details of the output.</p>
            </v-col>
            <v-col cols="7">
                <v-tabs fixed-tabs v-model="tab">
                <v-tab :ripple="false" href="#tab-details">Details</v-tab>
                <v-tab :ripple="false" href="#tab-analysis">Physical Progress</v-tab>
                <v-tab :ripple="false" href="#tab-payments">Financial Progress</v-tab>
                </v-tabs>
            </v-col>
            ...
            <v-tab-item value="tab-details">
              <v-container>
                <v-form ref="detailsForm">
                  <v-row>
                      <v-col cols="10">
                          <v-text-field label="Output/ Activity ID (Auto Generated)" v-model="autoGeneratedActivityId" readonly outlined/>
                      </v-col>
                      <v-col cols="10">
                          <v-text-field :rules="alphanumericRules" label="Output/ Activity Name *" v-model="activityName" outlined/>
                      </v-col>
                      <v-col cols="10">
                          <v-textarea :rules="requiredRule" v-model="activityDescription" label="Output/ Activity Description" outlined/>
                      </v-col>
                      <v-col cols="10" class="ma-0 pa-0 mb-1 pb-1 d-flex justify-space-between">
                          <v-card-subtitle class="my-auto">Does the Output/ Activity have sub-activities</v-card-subtitle>
                          <v-switch inset color="blue" v-model="subActivities"></v-switch>
                      </v-col>
                  </v-row>
                  <v-row v-if="!subActivities">
                      <v-col cols="10" class="d-flex">
                      <div class="d-flex">
                          <v-card-subtitle>Start Date :</v-card-subtitle>
                          <v-btn id="startDatePickerBtn" height="60" @click="startDateToggle = !startDateToggle">
                          ...
                          <v-date-picker v-model="startDate" v-if="startDateToggle" @input="startDateToggle = false" no-title class="ml-2">
                          </v-date-picker>
                      </div>
                      <div class="d-flex">
                          <v-card-subtitle>End Date :</v-card-subtitle>
                          <v-btn id="endDatePickerBtn" height="60" @click="endDateToggle = !endDateToggle">
                          ...
                          <v-date-picker v-model="endDate" :min="startDate" v-if="endDateToggle" @input="endDateToggle = false" no-title class="ml-2">
                          </v-date-picker>
                      </div>
                      </v-col>
                      <v-col cols="10">
                      <v-text-field :rules="[v => v>0 || 'Please select the start and end dates']" v-model="cWeightageValue" label="Weightage/ Duration (Auto calculated)" placeholder="30" outlined readonly/>
                      </v-col>
                      <v-col class="mb-0" cols="10">
                      <v-text-field :rules="alphanumericRules" v-model="activityUnit" label="Unit of measurement (UOM)" placeholder="Enter Unit" outlined/>
                      </v-col>
                      <v-col cols="10" class="ma-0 pa-0 mb-1 pb-1 d-flex justify-space-between">
                          ...
                          <v-switch inset color="blue" v-model="monthlyTarget"></v-switch>
                            ...
                            <!-- <v-row v-if="monthlyTarget"> -->
                                ...
                                <v-col v-if="monthlyTarget && startDate && endDate && activityName" cols="10" class="d-flex wrap-table mb-12">

                                    <table class="output-table" border="1" ref="DataGrid" style="table-layout: fixed ; border-collapse: collapse; width: max-content">
                                        <thead>
                                            <tr class="month_tr">
                                                <th  rowspan="2" class="sticky-col">Activity</th>
                                                <th rowspan="2"   >Cost/Unit(&#x20b9;)</th>

                                            <template v-for="(head, hid) in cHeaders">
                                        
                                              <th class="text-center" rowspan="1" colspan="2" :key="hid">{{head.monthYear}}</th>
                                            </template> 
                                            </tr>
                                            <tr>
                                            <template v-for="(head, hid) in cHeaders">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td class="sticky-col "><v-text-field readonly  class="td-text1"  solo flat hide-details v-model="activityName"></v-text-field></td>
                                                <td  contenteditable="true"><v-text-field :rules="numericRules" class="td-text2" solo flat hide-details v-model="unitCost"> </v-text-field></td>

                                            <template v-for="(head, hid) in cHeaders" >
                                            <!-- <template v-if="head.hasSubHeaders" > -->
                                            <td class="text-center" :key="hid"><v-text-field :rules="numericRules" v-model="head.target" class="td-text" hide-details solo flat> </v-text-field></td>
                                            <td class="text-center"><v-text-field readonly class="td-text" :value="head.budget=unitCost * head.target" solo flat hide-details> </v-text-field></td>
                                            <!-- </template>  -->
                    ...
                    <template v-if="monthlyTarget">
                      ...
                      <v-text-field  v-model="cactivityTarget" readonly label="Activity Target(Fill all Target cells to auto-calculate)" placeholder="10,0000" outlined/>
                      </v-col>  
                      <v-col cols="10">
                      <v-text-field  v-model="ctotalBudget" readonly label="Total Budget(Fill all table cells to auto-calculate)" placeholder="10,0000" outlined/>
                      </v-col>  
                    </template>   
                    <template v-if="monthlyTarget==false">
                      <v-col cols="10" class="pt-2">
                      <v-text-field :rules="numericRules" v-model="activityTarget" label="Activity Target"  placeholder="10,0000" outlined/>
                      </v-col>  
                      <v-col cols="10">
                      <v-text-field :rules="numericRule" v-model="totalBudget"  label="Total Budget" placeholder="10,0000" outlined/>
                      </v-col>  
                    </template>  
                         <!-- </v-row> -->
                    </v-row>
                      ...
                      <v-tab-item value="quantitative"> -->
                              ...
                              <v-col cols="12">
                                  ...
                                  <div class="d-flex align-center">
                                      <v-autocomplete
                                          ...
                                          :items="SurveyList"
                                          :item-text="z => z.SurveyName"
                                          :item-value="z => z.SurveyID"
                                          multiple 
                                          chips 
                                          outlined 
                                          dense 
                                          placeholder="Select multiple surveys" 
                                          return-object
                                      >
                                      </v-autocomplete>
                                      <v-btn tile depressed color="green" class="ml-1" text :ripple="false" @click="selectedSurveyList.length && (addSection = true), getSectionCollection()">+ Add Selection</v-btn>
                                  </div>
                              </v-col>
                              <v-col cols="12" v-if="addSection">
                                  <v-card-subtitle class="py-1" v-for="(item, index) in selectedSurveyList" :key="index">
                                      <v-hover v-slot="{ hover }" open-delay="100">
                                          <v-card-subtitle class="pa-0 ma-0 my-1">
                                              <span v-show="hover">
                                                  <v-btn x-small color="red" @click="selectedSurveyList.splice(index, 1)" :ripple="false" icon>
                          ...
                          <v-row class="d-flex flex-column mt-2">
                              ...
                              <v-row class="section-headers-wrapper justify-space-between align-center">
                                  <v-col cols="4" class="py-0 text-center">Select question under section</v-col>
                                  <v-col cols="3" class="py-0 text-center">Define logic between them</v-col>
                                  <v-col cols="4" class="py-0 text-center">Options under Selected question</v-col>
                                  <v-col cols="1"> </v-col>
                              </v-row>
                              <v-row>
                                  <v-col cols="4">
                                      <v-select 
                                          outlined
                                          dense
                                          label="Select Data Header"  
                                          v-model="sectionID"
                                          :items="cSelectSectionList"
                                      ...
                                      >
                                          ...
                                          <template v-slot:selection>
                                              <p class="my-auto">{{currentQuestion.QuestionName ? currentQuestion.QuestionName : 'Select Data Header'}}</p>
                                          </template>
                                      </v-select>
                                  </v-col>
                                  <v-col cols="3">
                                      <v-autocomplete
                                          ...
                                          dense
                                          label="Select Logic"
                                          :items="LogicItemsPrime"
                                          item-text="Type"
                                          item-value="ID"
                                          return-object
                                          v-model="SurveyLogicPrime"
                                      >
                                  ...
                                  <v-col cols="4">
                                      ...
                                      <v-text-field 
                                          ...
                                          dense
                                          label="Answer"
                                          v-model="SurveyLogicPrimeAnswer"
                                      />
                                  ...
                                  <v-col cols="1">
                                      <v-btn text color="green" class="ml-1" @click="addFilterSection()">+</v-btn>
                                  </v-col>
                              </v-row>

                              <v-row class="d-flex flex-column">
                                  <p class="mb-2">Selected conditions and set logic between them</p>
                                  <v-col v-if="!currentFilterSections.length" cols="8" class="section-conditions-wrapper my-1 d-flex justify-space-between px-5 align-center">
                                      <p class="ma-0 pa-0">Selected Sections <span class="mx-2"><v-icon>mdi-link</v-icon></span> Selected Question</p>
                                      ...
                                      <v-col :key="index" class="pa-1">
                                            <v-hover v-slot="{ hover }" open-delay="150">
                                                <div class="pa-0 ma-0 d-flex align-center">
                                                    <div class="section-conditions-subwrapper my-1 d-flex px-5 py-3 align-center justify-flex-start" style="width: fit-content;">
                                                        <p class="ma-0 pa-0 mr-5 font-weight-medium">{{item.Section.SectionName}} <span class="mx-2"><v-icon>mdi-link</v-icon></span> {{item.Question.QuestionName}}</p>
                                                        <p class="ma-0 pa-0 mr-5 font-weight-medium">{{item.Logic.Type}}</p>
                                                        <p class="ma-0 pa-0 mr-5 font-weight-medium">{{item.Answer}}</p>
                                                    </div>
                                                    <v-btn 
                                                        icon 
                                                        :key="index" 
                              ...
                              <v-row class="d-flex flex-column">
                                  ...
                                  <v-row class="section-headers-wrapper">
                                      <v-col cols="">
                                          <v-select
                                              ...
                                              dense 
                                              color="white" 
                                              placeholder="Pick a Mathematical Operation"
                                              :items="cLogicItems"
                                              v-model="currentActualsLogic"
                                              item-text="Type"
                                              item-value="ID"
                                      ...
                                      <v-col cols="">
                                          ...
                                          >
                                              ...
                                              <template v-slot:selection>
                                                  <p class="my-auto">{{currentAdditionalQuestion.QuestionName}}</p>
                                              </template>
                                          </v-select>
                                      </v-col>
                                      <!-- <v-col cols="">
                                  ...
                                  <v-row class="d-flex">
                                      <v-col cols="7">
                                          ...
                                          <v-text-field 
                                              v-model="actualsValue"
                                              outlined 
                                              placeholder="Non Editable" 
                                              dense
                                              persistent-hint
                                              readonly
                                              hint="The actual value is derived based on the summarised formula applied"
                                          ></v-text-field>
                                      </v-col>
                                      <v-col cols="2" align-self="center" class="pt-0">
                                        <v-btn   depressed @click="apply=1;validateFields();OutPutQuantitativeLogicList = []">Save & Apply</v-btn>
                                      </v-col>
                                  </v-row>
                          ...
                          <v-row>
                              <v-col cols="12">
                                  ...
                                  <div class="d-flex align-center">
                                      ...
                                      <v-btn tile depressed color="green" class="ml-1" text :ripple="false" @click="selectedSurveyList_f.length && (addSection_f = true), getSectionCollection_f()">+ Add Selection</v-btn>
                                  </div>
                              </v-col>
                              <v-col cols="12" v-if="addSection_f">
                                  <v-card-subtitle class="py-1" v-for="(item, index) in selectedSurveyList_f" :key="index">
                                      <v-hover v-slot="{ hover }" open-delay="100">
                                          <v-card-subtitle class="pa-0 ma-0 my-1">
                                              <span v-show="hover">
                                                  <v-btn x-small color="red" @click="selectedSurveyList_f.splice(index, 1)" :ripple="false" icon>
                                                      <v-icon small class="mr-2">mdi-close</v-icon>
                                                  </v-btn>
                                              </span>
                                              {{item.SurveyName}}
                          ...
                          <v-row class="d-flex flex-column mt-2">
                              ...
                              <v-row>
                                  <v-col cols="4">
                                      ...
                                      >
                                                  ...
                                                  >
                                                      ...
                                                      <v-list>
                                                          <v-list-item 
                                                              ...
                                                              :key="Qindex"
                                                              v-on="on"
                                                              @click="setQuestionSelection_f(Qitem, item)"
                                                          >
                                                              {{Qitem.QuestionName}}
                                                          </v-list-item>
                                                      </v-list>
                                          ...
                                          <template v-slot:selection>
                                              <p class="my-auto">{{currentQuestion_f.QuestionName ? currentQuestion_f.QuestionName : 'Select Data Header'}}</p>
                                          </template>
                                      </v-select>
                                  </v-col>
                                  <v-col cols="3">
                                      <v-autocomplete
                                          :disabled="!currentQuestion_f"
                                          outlined
                                          hide-details
                                          dense
                                          label="Select Logic"
                                          :items="LogicItemsPrime_f"
                                          item-text="Type"
                                          item-value="ID"
                                          return-object
                                          v-model="SurveyLogicPrime_f"
                                      >
                                  ...
                                  <v-col cols="4">
                                      ...
                                      <v-text-field 
                                          v-if="QuestionType_f == 0" 
                                          outlined
                                          clearable
                                          hide-details
                                          dense
                                          label="Answer"
                                          v-model="SurveyLogicPrimeAnswer_f"
                                      />
                                      
                                      <v-autocomplete 
                                          v-if="QuestionType_f == 1"
                                          outlined
                                          dense
                                          hide-details
                                          v-model="SurveyLogicPrimeAnswer_f"
                                          label="Select Answer"
                                          :items="lstLogicAnswers_f"
                                          item-text="OptionValue"
                                          item-value="OptionValue"
                                      >
                                      </v-autocomplete>
                                  ...
                                  <v-col cols="1">
                                      <v-btn text color="green" class="ml-1" @click="addFilterSection_f()">+</v-btn>
                                  </v-col>
                              </v-row>

                              <v-row class="d-flex flex-column">
                                  <p class="mb-2">Selected conditions and set logic between them</p>
                                  <v-col v-if="!currentFilterSections_f.length" cols="8" class="section-conditions-wrapper my-1 d-flex justify-space-between px-5 align-center">
                                      <p class="ma-0 pa-0">Selected Sections <span class="mx-2"><v-icon>mdi-link</v-icon></span> Selected Question</p>
                                      <p class="ma-0 pa-0">Equal to</p>
                                      <p class="ma-0 pa-0">Output</p>                      
                                  </v-col>
                                  <template v-else v-for="(item, index) in currentFilterSections_f">
                                      <v-col :key="index" class="pa-1">
                                            <v-hover v-slot="{ hover }" open-delay="150">
                                                <div class="pa-0 ma-0 d-flex align-center">
                                                    <div class="section-conditions-subwrapper my-1 d-flex px-5 py-3 align-center justify-flex-start" style="width: fit-content;">
                                                    ...
                                                    <v-btn 
                                                        ...
                                                        v-show="hover" 
                                                        class="ml-3"
                                                        @click="currentFilterSections_f.splice(index, 1)"
                                                    >
                                                        <v-icon color="red">
                                                            mdi-delete-outline
                                                        </v-icon>
                                            ...
                                            <v-col cols="2">
                                                <v-select 
                                                    ...
                                                    solo 
                                                    dense 
                                                    :items="lstConditions_f"
                                                    v-if="index < currentFilterSections_f.length-1" 
                                                    item-text="OptionValue"
                                                    item-value="OptionID"
                                                    hide-details
                                                    return-object
                              ...
                              <v-row class="d-flex flex-column">
                                  ...
                                  <v-row class="section-headers-wrapper">
                                      ...
                                      <v-col cols="">
                                          <v-select 
                                              v-model="additionalSection_f"
                                              :disabled="currentActualsLogic_f==1"
                                              clearable  
                                              solo 
                                              flat 
                                              dense 
                                              color="white"
                                              :items="cAdditionalSectionList_f"
                                              item-text="SectionName"
                                              item-value="SectionId"
                                              return-object
                                              placeholder="Select Question Under Section"
                                          ...
                                          >
                                              <template v-slot:prepend-item>
                                                  <v-list-item class="my-2">
                                                      <v-text-field 
                                                          ...
                                                          outlined
                                                          label="search"
                                                          v-model="additionalSectionSearch_f"
                                                      />
                                                  </v-list-item>
                                                  <v-divider></v-divider>
                                              </template>
                                                      ...
                                                      >
                                                          ...
                                                          <v-list>
                                                              <v-list-item 
                                                                  ...
                                                                  :key="Qindex"
                                                                  v-on="on"
                                                                  @click="setAdditionalQuestionSelection_f(Qitem, item)"
                                                              >
                                                                  {{Qitem.QuestionName}}
                                                              </v-list-item>
                                                          </v-list>
                                              ...
                                              <template v-slot:selection>
                                                  <p class="my-auto">{{currentAdditionalQuestion_f.QuestionName}}</p>
                                              </template>
                                          </v-select>
                                      </v-col>
                                      <!-- <v-col cols="">
                                          <v-select  
                                              v-model="currentComparison_f"
                                              :items="comparisonList_f"
                                              item-text="name"
                                              item-value="id"
                                              ...
                                              <template v-slot:prepend-item>
                                                  <v-list-item class="mb-2">
                                                      <v-text-field
                                                          outlined
                                                          prepend-inner-icon="mdi-magnify"
                                                          v-model="comparisonListSearch_f"
                                                          hide-details
                                                          label="Search"
                                                          dense
                                                      >
                                  ...
                                  <v-row class="d-flex ">
                                      ...
                                      <v-col  cols="2" align-self="center" class="pt-0">
                                        <v-btn   depressed @click="apply=2;validateFields();OutPutQuantitativeLogicList = []">Save & Apply</v-btn>
                ...
                <!-- <v-form ref="financialForm">
                  <v-row >
                      <v-col cols="7" class="d-flex justify-space-between">
                          <p class="mb-0 px-2">Is the Activity linked to Payment</p>
                          <v-switch v-model="paymentLink" inset color="blue" class="py-0 my-0"></v-switch>
                      </v-col>
                  </v-row>

                  <v-row v-if="paymentLink">
                      <v-col cols="7" class="my-0 py-0">
                          <p class="mb-1 px-2">Budget Amount</p>
                          <v-text-field :rules="budjetRules" v-model="projectBudgetAmount" outlined placeholder="50,000"></v-text-field> 
                      </v-col>
                  </v-row>
                </v-form>   -->
              </v-container>
        ...
        <v-card-actions class="align-self-end">
          <!-- <Success-Button :title="tab == 'tab-payments' ? 'Save & Complete' : 'Next'" @click.native="tab == 'tab-payments' ? fnCreateActivity() : (tab == 'tab-details' ? ( validateFields()) : (fnAggregateAnswers(),tab = 'tab-payments'))"></Success-Button> -->
          <!-- <Success-Button :title="tab == 'tab-payments' ? 'Save & Complete' : 'Next' " @click.native= validateFields()></Success-Button> -->
...
export default {
  ...
  data() {
    return {
      ...
      currentQuestion_f: null,
      currentQuestionParent: null,
      currentQuestionParent_f: null,
      selectLogicSearch: '',
      QuestionType: 0,
      QuestionType_f: 0,

      currentFilterSections: [],
      currentFilterSections_f: [],
      currentActualsLogic: '',
      currentActualsLogic_f: '',
      additionalSectionSearch: '',
      additionalSection: '',
      additionalSectionSearch_f: '',
      additionalSection_f: '',
      currentAdditionalQuestion: null,
      currentAdditionalQuestionParent: null,
      currentAdditionalQuestion_f: null,
      currentAdditionalQuestionParent_f: null,
      currentComparison: 1,
      currentComparison_f:1,
      paymentLink: false,
      ...
      comparisonListSearch_f: '',
      LogicItemsSearch: '',
      LogicItemsSearch_f: '',
      currentQuestionPayload: [],
      currentQuestionPayload_f: [],
      ...
      SurveyLogicPrime_f: '',
      SurveyLogicPrimeAnswer_f: '',
    
      OutPutQuantitativeLogicList : [],
      OutPutQuantitativeLogicList_f : [],
      lstLogicAnswers: [],
      lstLogicAnswers_f: [],
      masterSectionCollection: [],
      masterSectionCollection_f: [],
      requiredRule: [
                    (v) => !!v || "Required field.",
      ],
      alphanumericRules: [
  ...
  async mounted() {
    ...
    if(this.isSub)
    {
        this.DialogHeader = 'Sub-Activity'
    }
    else{
  ...
  computed: {
    ...
    cSelectSectionList_f() {
        if(!this.selectedSurveyList_f || !this.selectedSurveyList_f.length) return []
        let data = []
        let surveyIdList = this.selectedSurveyList_f.map(x => x.SurveyID)
        this.masterSectionCollection_f.forEach((x) => {
            if(surveyIdList.includes(x.SurveyId)){
                data.push(...x.ProjectSurveySectionList)
            }
        })
        ...
        if(this.selectDataSearch_f) {
            return this.data.filter(x => x.SectionName.includes(this.selectDataSearch_f))
        }
        else return data
    },
    ...
    cAdditionalSectionList_f() {
        if(!this.selectedSurveyList_f || !this.selectedSurveyList_f.length) return []
        let data = this.sectionCollection_f.map(x => {
            return {
                ...x,
                ProjectSurveySectionQuestionsList: x.ProjectSurveySectionQuestionsList.filter(s => s.Type && s.Type == 'Numeric')
            }
        ...
        data = data.filter(x => x.ProjectSurveySectionQuestionsList.length)
        if(this.additionalSectionSearch_f){
            return data.filter(x => x.SectionName.includes(this.additionalSectionSearch_f))
        }
        else return data
    },
    ...
    cComparisonList_f () {
        if(this.comparisonListSearch_f) {
            return this.comparisonList_f.filter(x => x.name.includes(this.comparisonListSearch_f))
        }

        else return this.comparisonList_f
    },
    cLogicItems () {
    ...
    cLogicItems_f () {
        if(this.LogicItemsSearch_f) {
            return this.LogicItems_f.filter(x => x.Type.includes(this.LogicItemsSearch_f))
        }

        else return this.LogicItems_f
    },
    cWeightageValue: {
        get(){
  ...
  methods: {
    async fnCreateActivity() {
        ...
        data.append('ParentActivityId', this.parentActivityID)
        data.append('ActivityName', this.activityName)
        data.append('Description', this.activityDescription)
        data.append('isSubActivitiesAvailable', this.subActivities)
        // if subactivities toggle
        if(!this.subActivities){
            data.append("StartDate", this.startDate);
            data.append("EndDate", this.endDate);
            data.append("WeightagePercentage", this.weightageValue);
        }

        if(this.headers && this.headers.length && this.monthlyTarget) {
            for (var x = 0; x < this.headers.length; x++)
              {
                data.append('lstProjectActivityMonthwiseData[' + x + '].ActivityID', this.activityID);
                data.append('lstProjectActivityMonthwiseData[' + x + '].Target', this.headers[x].target);
                data.append('lstProjectActivityMonthwiseData[' + x + '].Budget', this.headers[x].budget);
                data.append('lstProjectActivityMonthwiseData[' + x + '].ActMonth', this.headers[x].actMonth);
                data.append('lstProjectActivityMonthwiseData[' + x + '].ActYear', this.headers[x].actYear);       
        ...
        data.append("ActivityType", this.activityType);
        data.append("FinancialActivityType", this.activityType);

        data.append("SurveyId", 0);
        if(this.activityType == 'Quantitative') {
            if(this.QuestionCategory == 'Section') {
            ...
            if(this.QuestionCategory == 'Beneficiary') {
                data.append("BeneficiaryTypeQuestionID", this.BeneficiaryTypeQuestionID);
            ...
            data.append("FinancialSurveyLogic", this.currentActualsLogic_f);
            data.append("FinancialSurveyTarget", this.activityTarget?this.activityTarget:0);
            let i=0
            ...
            if(this.OutPutQuantitativeLogicList && this.OutPutQuantitativeLogicList.length) {
                 for ( i = 0; i < this.OutPutQuantitativeLogicList.length; i++) {
                    data.append('OutPutQuantitativeLogic[' + i + '].QuestionId', this.OutPutQuantitativeLogicList[i].QuestionId);
                    data.append('OutPutQuantitativeLogic[' + i + '].SectionId', this.OutPutQuantitativeLogicList[i].SectionId);
                    data.append('OutPutQuantitativeLogic[' + i + '].SurveyId', this.OutPutQuantitativeLogicList[i].SurveyId);
                    data.append('OutPutQuantitativeLogic[' + i + '].BeneficiaryTypeQstnID', this.OutPutQuantitativeLogicList[i].BeneficiaryTypeQstnID);
                    data.append('OutPutQuantitativeLogic[' + i + '].TypeofLogic', this.OutPutQuantitativeLogicList[i].TypeofLogic);
                    ...
                    if (this.OutPutQuantitativeLogicList[i].SurveyPrimeLogic != null && this.OutPutQuantitativeLogicList[i].SurveyPrimeLogic != undefined && this.OutPutQuantitativeLogicList[i].SurveyPrimeLogic.length > 0) {
                        for (var k = 0; k < this.OutPutQuantitativeLogicList[i].SurveyPrimeLogic.length; k++) {
                            data.append('OutPutQuantitativeLogic[' + i + '].lstSurveyPrimeLogic[' + k + '].SurveyLogicPrime', this.OutPutQuantitativeLogicList[i].SurveyPrimeLogic[k].SurveyLogicPrimeID);
                            data.append('OutPutQuantitativeLogic[' + i + '].lstSurveyPrimeLogic[' + k + '].SurveyLogicPrimeAnswer', this.OutPutQuantitativeLogicList[i].SurveyPrimeLogic[k].SurveyLogicPrimeAnswer);
                            data.append('OutPutQuantitativeLogic[' + i + '].lstSurveyPrimeLogic[' + k + '].SurveyLogicCondition', this.OutPutQuantitativeLogicList[i].SurveyPrimeLogic[k].SurveyLogicCondition);
                        }
                    }
                }
            ...
            if(this.OutPutQuantitativeLogicList_f && this.OutPutQuantitativeLogicList_f.length) {
                 for (var j = 0; j < this.OutPutQuantitativeLogicList_f.length; j++) {
                    data.append('OutPutQuantitativeLogic[' + i + '].QuestionId', this.OutPutQuantitativeLogicList_f[j].QuestionId);
                    data.append('OutPutQuantitativeLogic[' + i + '].SectionId', this.OutPutQuantitativeLogicList_f[j].SectionId);
                    data.append('OutPutQuantitativeLogic[' + i + '].SurveyId', this.OutPutQuantitativeLogicList_f[j].SurveyId);
                    data.append('OutPutQuantitativeLogic[' + i + '].BeneficiaryTypeQstnID', this.OutPutQuantitativeLogicList_f[j].BeneficiaryTypeQstnID);
                    data.append('OutPutQuantitativeLogic[' + i + '].TypeofLogic', this.OutPutQuantitativeLogicList_f[j].TypeofLogic);

                    if (this.OutPutQuantitativeLogicList_f[j].SurveyPrimeLogic != null && this.OutPutQuantitativeLogicList_f[j].SurveyPrimeLogic != undefined && this.OutPutQuantitativeLogicList_f[j].SurveyPrimeLogic.length > 0) {
                        for (var k = 0; k < this.OutPutQuantitativeLogicList_f[j].SurveyPrimeLogic.length; k++) {
                            data.append('OutPutQuantitativeLogic[' + i + '].lstSurveyPrimeLogic[' + k + '].SurveyLogicPrime', this.OutPutQuantitativeLogicList_f[j].SurveyPrimeLogic[k].SurveyLogicPrimeID);
                            data.append('OutPutQuantitativeLogic[' + i + '].lstSurveyPrimeLogic[' + k + '].SurveyLogicPrimeAnswer', this.OutPutQuantitativeLogicList_f[j].SurveyPrimeLogic[k].SurveyLogicPrimeAnswer);
                            data.append('OutPutQuantitativeLogic[' + i + '].lstSurveyPrimeLogic[' + k + '].SurveyLogicCondition', this.OutPutQuantitativeLogicList_f[j].SurveyPrimeLogic[k].SurveyLogicCondition);
                        }
                    }
                    i++
                }
        ...
        data.append("ActivityUsersList", null);
        ...
        data.append("FinancialAdditionalSectionId", this.additionalSection_f == undefined ? 0 : this.additionalSection_f.SectionId == undefined ? 0 : this.additionalSection_f.SectionId);
        data.append("FinancialAdditionalQuestionId", this.currentAdditionalQuestion_f == null ? 0 : this.currentAdditionalQuestion_f.QuestionId);
        data.append("FinancialAdditionalQuestionType", true); 
        data.append("UnitCost",this.unitCost);

        try {
            let result = await objManageProjectActivity.AddActivity(data);
            if(result.Status == 'Unhandled Exception' || result.Status == 'PeriodicityFailed'){
                ...
                if(this.apply==1)
                {
                    let res=await objManageProjectActivity.GetProjectActivityPhysicalActualValues(result.ActivityId)
                ...
                {
                    let res_fin=await objManageProjectActivity.GetProjectActivityActualValues(result.ActivityId)
                    if (res_fin.data) {
                        this.actualsValue_f=res_fin.data.FinancialActualValue
                        this.apply=0
                    } 
                }
                else{
        ...
        this.OutPutQuantitativeLogicList_f = []
    ...
    async getSectionCollection_f() {
        if(!this.selectedSurveyList_f.length) {
            return
        }

        this.sectionCollection_f = []
        ...
        try {
            ...
            if(!data || !data.length) {
                ...
                data.forEach(q => this.sectionCollection_f.push(...q.ProjectSurveySectionList))
                this.sectionCollection_f = this.sectionCollection_f.map(x => {
                    return {
                        ...x,
                        ProjectSurveySectionQuestionsList: x.ProjectSurveySectionQuestionsList.filter(s => s.Type)
                    }
                })
                this.masterSectionCollection_f = data
            }
            return
        } catch (error) {
            console.error(error)
      ...
      if (result.status == "Unhandled Exception") {
          ...
          if (result.data != null && result.data != undefined) {
            // console.log(result.data)
              ...
              this.LogicItems_f = result.data.filter(x => x.IsPrimeLogic == false);
              this.LogicItemsPrime_f = result.data.filter(x => x.IsPrimeLogic == true);
              this.LogicItemsPrimeCopy_f = result.data.filter(x => x.IsPrimeLogic == true);
          }
      }
    },

    async GetProjectActivityActualValues() {
      let result = await objManageProjectActivity.GetProjectActivityActualValues(this.activityID);
     if (result.data) {
        console.log(result.data)
          // this.filterPaymentList = result.data;
          //this.actualsValue = result.data.PhysicalActualValue;
           this.actualsValue_f=result.data.FinancialActualValue
      } else {
    ...
    async initializeEdit() {
      ...
      let {
        ...
        WeightagePercentage,
        UnitOfMeasure,
        isSubActivitiesAvailable,
        UnitCost,
        lstProjectActivityMonthwiseData,
        FinancialSurveyLogic,
        FinancialSurveyTarget,
        FinancialAdditionalSectionId,
        FinancialAdditionalQuestionId,
        PhysicalActualValue
      ...
      this.subActivities = isSubActivitiesAvailable != undefined ? isSubActivitiesAvailable : false
      this.unitCost=UnitCost
      this.currentActualsLogic_f=FinancialSurveyLogic
      this.actualsValue= PhysicalActualValue
      let surveyIdList  = OutPutQuantitativeLogic.map(x =>  { if(x.TypeofLogic==1){ return x.SurveyId}})
      let surveyIdList_f = OutPutQuantitativeLogic.map(x => { if(x.TypeofLogic==2){ return x.SurveyId}})
      this.headers_edit =lstProjectActivityMonthwiseData
      if(this.headers_edit && this.headers_edit.length){
      ...
      this.selectedSurveyList_f = this.SurveyList.filter(x => {
          if(surveyIdList_f.includes(x.SurveyID)){
              return x
          }
      })
      ...
      await this.getSectionCollection_f()
      if(this.sectionCollection_f.length) {
        if(FinancialAdditionalSectionId>=1){

        this.additionalSection_f = this.sectionCollection_f.find(x => x.SectionId == FinancialAdditionalSectionId)
        this.currentAdditionalQuestionParent_f = this.additionalSection_f
        this.currentAdditionalQuestion_f = this.additionalSection_f.ProjectSurveySectionQuestionsList.find(x => x.QuestionId == FinancialAdditionalQuestionId
)
        }
        OutPutQuantitativeLogic.map(x => {
            if(x.TypeofLogic==2)
            {
                this.currentFilterSections_f.push({
                    Question: {
                        QuestionName: x.QuestionName,
                        QuestionId: x.QuestionId==0?x.BeneficiaryTypeQstnID:x.QuestionId,
                        SectionORBeneficiary: x.QuestionId==0?"Beneficiary":"Section",                    },
    ...
    async setQuestionSelection_f(item, parent) {
        this.Loader = true
        this.QuestionType_f = 0
        this.LogicItemsPrime_f = this.LogicItemsPrimeCopy_f
        this.SurveyLogicPrimeAnswer_f = ''
        this.currentQuestion_f = null
        this.currentQuestionParent_f = null
        if(item) {
            this.currentQuestion_f = item
            this.currentQuestionParent_f = parent
            let { QuestionId, QuestionTypeID,SectionORBeneficiary, Type} = item
            // Update logic list
            if(QuestionId && Type) {
                if(Type != 'Numeric' && Type != 'Date Picker'){
                    this.LogicItemsPrime_f = this.LogicItemsPrimeCopy_f.filter(x => x.IsNonNumeric)
                } else {
                    this.LogicItemsPrime_f = this.LogicItemsPrimeCopy_f
                }

                if(['Single Choice', 'Multi-Choice', 'Dropdown' ,'Likert Scale'].includes(Type)){
                    this.QuestionType_f = 1
                    if(SectionORBeneficiary=="Beneficiary"){
                        try {
                            let { data } = await objManageProjectActivity.GetBeneficiaryAnswerOption(QuestionId)
                            if(!data || !data.length) {
                ...
                if(Type == 'Date Picker') {
                    this.QuestionType_f = 2
                }

                if(Type == 'Cascading Dropdown') {
                    this.QuestionType_f = 3
    ...
    setAdditionalQuestionSelection_f (item, parent) {
        this.PreLoader = true
        this.currentAdditionalQuestion_f = ''
        this.currentAdditionalQuestionParent_f = ''
        if(item) {
            this.currentAdditionalQuestion_f = item
            this.currentAdditionalQuestionParent_f = parent
        }
        this.PreLoader = false
    },
    ...
    addFilterSection_f() {
        if(this.currentFilterSections_f.length) {
            let data = this.currentFilterSections_f.find(x => x.FilterID == `${this.currentQuestionParent_f.SectionId}${this.currentQuestion_f.QuestionId}${this.SurveyLogicPrime_f.ID}${this.SurveyLogicPrimeAnswer_f}`)
            if(data) {
                return
            }
        }
        ...
        let payload = {
            FilterID: `${this.currentQuestionParent_f.SectionId}${this.currentQuestion_f.QuestionId}${this.SurveyLogicPrime.ID}${this.SurveyLogicPrimeAnswer_f}`,
            Section: this.currentQuestionParent_f,
            Question: this.currentQuestion_f,
            Logic: this.LogicItemsPrime_f.find(x => x.ID == this.SurveyLogicPrime_f.ID),
            Answer: this.SurveyLogicPrimeAnswer_f,
            condition: this.lstConditions_f[0]
        }
        ...
        this.currentFilterSections_f.push({...payload, SurveyPrimeLogic: primeLogic})

        this.$on('conditionChanged', (val) => {
            let el = this.currentFilterSections_f.find(x => x.FilterID == val)
            el.SurveyPrimeLogic[0].SurveyLogicCondition = el.condition.OptionID
        })
        
        this.currentQuestion_f = {}
        this.currentQuestionParent_f = {}
        this.SurveyLogicPrime_f = 0
        this.SurveyLogicPrimeAnswer_f = ''
    },
    ...
    fnAggregateAnswers_f() {
        this.currentFilterSections_f.forEach(x => {
            let master
            this.masterSectionCollection_f.find(s => {
                s.ProjectSurveySectionList &&
                s.ProjectSurveySectionList.forEach(f => {
                    if(f.SectionId == x.Section.SectionId) {
                        master = s.SurveyId
            ...
            if(master) {             
                this.OutPutQuantitativeLogicList_f.push({
                    QuestionId:  x.Question.SectionORBeneficiary=="Beneficiary"? 0:x.Question.QuestionId,
                    QuestionName: x.Question.QuestionName,
                    SectionId: x.Section.SectionId,
                    SectionName: x.Section.SectionName,
  ...
  //  function to validate fields
    ...
    //                 this.tab = 'tab-analysis'
    //             }  
    //             break;
    //         case 'tab-analysis':
    //             if(this.$refs.physicalForm.validate()){
    //                 if(this.currentFilterSections.length){
    //                   this.fnAggregateAnswers();
    //                   this.tab= 'tab-payments'
    //                 }
    //                 else{
    //                 // this.snackBarMsg.message = "Please add filter condition"
    //                 // this.snackBarMsg.value = true
    //                 // this.snackBarMsg.color = 'error'
    //                 this.tab= 'tab-payments'
    ...
    //             break;
    //         case 'tab-payments':
    //             if(this.$refs.detailsForm.validate() && this.$refs.physicalForm.validate() && this.$refs.financialForm.validate()){
    //                  if(this.currentFilterSections_f.length){
    //                   this.fnAggregateAnswers_f();
    //                  }
    //                     this.fnCreateActivity() 
    //             }
    //             else{
    //                 this.snackBarMsg.message = "Please fill all mandatory details in all tabs"
    //                 this.snackBarMsg.value = true
    //                 this.snackBarMsg.color = 'error'
    //             }
    //             break;
  ...
  watch: {
    ...
    selectedSurveyList_f: function(v) {
      if(!v.length){
        this.addSection_f = false
      }
    },
    tab: function(v) {
        if(v !== 'tab-payments'){
