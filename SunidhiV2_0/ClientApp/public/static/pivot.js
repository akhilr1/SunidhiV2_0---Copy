  ...
  callWithJQuery(function($) {
      ...
      for (i in rowKeys) {
        ...
        for (j in rowKey) {
          ...
          if (x !== -1) {
            ...
            var isGPS = txt.indexOf('maps/search') !== -1 ? true : false;
            //var allowedExtensions = /(\jpg|\jpeg|\png|\gif|\txt|\mp3|\pdf|\doc|\docx|\xlsx)$/i;
            var allowedExtensions = ['jpg', 'jpeg', 'gif', 'png', 'txt', 'mp3', 'pdf', 'doc', 'docx', 'xlx', 'xlsx', 'csv', 'sveg','zip','rar'];
            let filextension = txt.split('.').pop();
            const validURLPattern = /^(?:\w+:)?\/\/([^\s\.]+\.\S{2}|localhost[\:?\d]*)\S*$/;
            var isValidURL = validURLPattern.test(txt);
            if (isValidURL && (allowedExtensions.indexOf(filextension) >= 0 || isGPS))
            {
                var link = document.createElement('a');//create link
                link.setAttribute('href', txt);//set href
                if (isGPS) {
                    // txt = https://www.google.com.sa/maps/search/10.054982527365764,76.29459825893557?hl=en
                    var corordinate = txt.split('/').pop();
                    link.innerHTML = corordinate.split('?')[0];
                }
                else
                {
                    link.innerHTML = txt.split('/').pop();//set text to be seen. Only file name needs to display
                }
                link.target = '_blank';
                th.innerHTML = ''; //Now hyperlink only required. Need to remove value set from calling function like appendFilePath() in ManageProjectSurveysIndividual.vue
                th.appendChild(link);
            }
    ...
    $.fn.pivotUI = function(input, inputOpts, overwrite, locale) {
      ...
      try {
        ...
        rendererControl = $("<td>").addClass("pvtUiCell pvtUiCell_tr1_td1");
          renderer = $("<select>").css('-webkit-appearance','menulist').appendTo(rendererControl).bind("change", function() {
          return refresh();
        });
        ref = opts.renderers;
        for (x in ref) {
          if (!hasProp.call(ref, x)) continue;
            if (x == inputOpts.rendererName) {
                $("<option>").val(x).attr("selected", true).html(x).appendTo(renderer);
            }
            else {
        ...
        fn1 = function(attr) {
          ...
          if (values.length > opts.menuLimit) {
            ...
            if (values.length > 5) {
              ...
              $("<button>", {
                type: "button"
              }).appendTo(controls).html(opts.localeStrings.selectAll).addClass("applyButton").bind("click", function() {
                valueList.find("input:visible:not(:checked)").prop("checked", true).toggleClass("changed");
                return false;
              });
              $("<button>", {
                type: "button"
              }).appendTo(controls).html(opts.localeStrings.selectNone).addClass("applyButton").bind("click", function() {
          ...
          if (values.length <= opts.menuLimit) {
            $("<button>", {
              type: "button"
            }).text(opts.localeStrings.apply).addClass("applyButton").appendTo(finalButtons).bind("click", function() {
          ...
          $("<button>", {
            type: "button"
          }).text(opts.localeStrings.cancel).addClass("cancelButton").appendTo(finalButtons).bind("click", function() {
        ...
        tr1 = $("<tr>").appendTo(uiTable);
          aggregator = $("<select>").css('-webkit-appearance', 'menulist').addClass('pvtAggregator').bind("change", function() {
          return refresh();
        });
        ...
        rowOrderArrow = $("<a>", {
          role: "button", title:"row order sorting"
        ...
        colOrderArrow = $("<a>", {
            role: "button", title: "col order sorting"
        }).addClass("pvtColOrder").data("order", opts.colOrder).html(ordering[opts.colOrder].colSymbol).bind("click", function() {
          $(this).data("order", ordering[$(this).data("order")].next);
          $(this).html(ordering[$(this).data("order")].colSymbol);
          return refresh();
        });
        $("<td>").addClass('pvtVals pvtUiCell pvtUiCell_tr1_td2').appendTo(tr1).append(aggregator).append(rowOrderArrow).append(colOrderArrow).append($("<br>"));
