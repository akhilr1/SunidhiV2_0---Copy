<template>
    <v-card>
        ...
        <v-container fluid style="min-height:70vh">
            <v-row>
                ...
                <v-col md="auto">
                    <v-row>
                        ...
                        <!--<v-btn id="AutoTest_MP49" height="39" color="success-btn" dark class="elevation-0 mr-4" @click="CreateProjectDialog=true">Create Project</v-btn>-->
                        <!--Bulk Action -->
                        <v-menu offset-y transition="scroll-y-transition" v-bind:disabled="!screenAccess">
                            ...
                            <v-list color="white" light="" width="auto" max-width="250" class="action-button-list">
                                ...
                                <v-list-item id="AutoTest_MP50" @click.stop="fnBulkRemoveClick()">
                                    <v-list-item-icon>
                            ...
                            <v-dialog v-model="RemoveBulkProjectDailog" width="800">
                                <v-card>
                                    <v-card-title primary-title class="page-head py-4">
                                        ...
                                        Remove Project
                                        <v-spacer></v-spacer>
                                        <v-btn id="AutoTest_MP51" text="" @click="RemoveBulkProjectDailog=false">
                                                    ...
                                                    <v-container fluid class="pt-0">
                                                        <v-row>
                                                            ...
                                                            <v-col cols="12" xs="12" sm="12" md="12" class="text-right py-0">
                                                                <v-btn id="AutoTest_MP52" color="secondary elevation-0" class="mr-2" @click="DeleteProject(1)">Remove</v-btn>
                                                                <v-btn id="AutoTest_MP53" outlined color="grey" @click="RemoveBulkProjectDailog=false">Cancel</v-btn>
                                                            </v-col>
                                                        </v-row>
                                                    </v-container>
                                                </v-col>
            ...
            <!--Main Body-->
            <v-row v-if="FilterProjectTableItems.length>0" class="px-0">
                ...
                <v-col cols="12" md="12" sm="12" xs="12" class="px-0">
                    <v-data-table :headers="ProjectTableheaders"
                        ...
                        <template v-slot:item.ProjectName="{ item }">
                            <router-link class="text--default" link :to="{ name: 'MyprojectsDeployedIndividual', query: { WsID: item.WorkspaceID,Access:RWAccess, InitID: InitiativeID, ProjID: item.ProjectId,AccessType:'MyProject' } }">
                        ...
                        <template v-slot:item.Actions="{item}">
                            <v-menu close-on-content-click offset-y left transition="scroll-y-transition" v-bind:disabled="!screenAccess">
                                ...
                                <v-list color="white" light width="auto" class="action-button-list">
                                    ...
                                    <v-list-item id="AutoTest_MP54" @click.stop="" link :to="{ name: 'MyprojectsDeployedIndividual', query: { WsID: item.WorkspaceID,Access:RWAccess, InitID: InitiativeID, ProjID: item.ProjectId,AccessType:'MyProject' } }">
                                        <v-list-item-icon>
                                    ...
                                    <v-list-item id="AutoTest_MP55" link @click.stop="GetProjectID(item.ProjectId,item.ProjectName),ArchiveDailog=true,ProjectName=item.ProjectName">
                                        <v-list-item-icon>
                                    ...
                                    <v-list-item id="AutoTest_MP56" link @click.stop="GetProjectID(item.ProjectId,item.ProjectName),ForceCompleteDailog=true,ProjectName=item.ProjectName">
                                        <v-list-item-icon>
                                    ...
                                    <v-list-item id="AutoTest_MP57" link @click.stop="GetProjectID(item.ProjectId,item.ProjectName),GetProjectTemplateByprjtID(),GetProjectTheme()">
                                        <v-list-item-icon>
                                    ...
                                    <v-list-item id="AutoTest_MP58" link="" @click.stop="GetProjectID(item.ProjectId,item.ProjectName),RemoveProjectDailog=true,ProjectName=item.ProjectName">
                                        <v-list-item-icon>
                                            <v-icon>mdi-trash-can-outline</v-icon>
                                        </v-list-item-icon>
                                        <v-list-item-content>
                                ...
                                <v-dialog v-model="ArchiveDailog" width="800">
                                    <v-card>
                                        <v-card-title primary-title class="page-head py-4">
                                            ...
                                            Archive Project
                                            <v-spacer></v-spacer>
                                            <v-btn id="AutoTest_MP59" text="" @click="ArchiveDailog=false">
                                                <v-icon color="#707070" size="15">mdi-close</v-icon>
                                            </v-btn>
                                        </v-card-title>
                                        <v-card-text class="pt-0">
                                                            ...
                                                            <v-row>
                                                                ...
                                                                <v-col>
                                                                    <h4 class="text-center heading-3">
                                                                        <v-avatar class="mr-2 elevation-1" color="#fff">
                                                                            <v-icon color="#707070" size="25">mdi-file-document-box-minus-outline</v-icon>
                                                                        </v-avatar>
                                                                        {{ProjectName}}
                                                                    </h4>
                                                                </v-col>
                                                                ...
                                                                <v-col cols="12" xs="12" sm="12" md="12" class="text-right py-0">
                                                                    <Secondary-Button id="AutoTest_MP60" class="btn-122x36 mr-4" title="Archive" @click.native="ChangeProjectStatus('Archived')"></Secondary-Button>
                                                                    <Default-Button-Outlined id="AutoTest_MP61" class="btn-122x36" title="Cancel" @click.native="ArchiveDailog=false"></Default-Button-Outlined>
                                                                </v-col>
                                                            </v-row>
                                                        </v-container>
                                                    </v-col>
                                ...
                                <!-- Force to complete -->
                                <v-dialog v-model="ForceCompleteDailog" width="800">
                                    <v-card>
                                        <v-card-title primary-title class="page-head py-4">
                                            <v-avatar color="white" size="25" class="elevation-1 mr-2">
                                                <v-icon size="20" color="primary">mdi-arrow-up-bold-circle-outline</v-icon>
                                            </v-avatar>
                                            Force Complete
                                            <v-spacer></v-spacer>
                                            <v-btn id="AutoTest_MP62" text="" @click="ForceCompleteDailog=false">
                                                <v-icon color="#707070" size="15">mdi-close</v-icon>
                                            </v-btn>
                                        </v-card-title>
                                        <v-card-text class="pt-0">
                                                ...
                                                <v-row no-gutters>
                                                    <v-col cols="12" md="12" class="pt-0">
                                                        <v-container fluid class="pt-0">
                                                            <v-row>
                                                                <v-col cols="12" md="12" class="pt-0">
                                                                    <h4 class="text-center heading-3">Are you sure you want to force complete this project ?</h4>
                                                                </v-col>
                                                                <v-col>
                                                                    <h4 class="text-center heading-3">
                                                                        <v-avatar class="mr-1 elevation-1" color="#fff" size="40">
                                                                            <v-icon color="#707070" size="25">mdi-arrow-up-bold-circle-outline</v-icon>
                                                                        </v-avatar>
                                                                        {{ProjectName}}
                                                                    </h4>
                                                                </v-col>
                                                                ...
                                                                <v-col cols="12" xs="12" sm="12" md="12" class="text-right py-0">
                                                                    <Secondary-Button id="AutoTest_MP63" class="btn-122x36 mr-4" title="Confirm" @click.native="ChangeProjectStatus('Completed')"></Secondary-Button>
                                                                    <Default-Button-Outlined id="AutoTest_MP64" class="btn-122x36" title="Cancel" @click.native="ForceCompleteDailog=false"></Default-Button-Outlined>
                                                                </v-col>
                                                            </v-row>
                                                        </v-container>
                                                    </v-col>
                                ...
                                <v-dialog v-model="RemoveProjectDailog" width="800">
                                    <v-card>
                                        <v-card-title primary-title class="page-head py-4">
                                            ...
                                            Remove Project
                                            <v-spacer></v-spacer>
                                            <v-btn id="AutoTest_MP65" text="" @click="RemoveProjectDailog=false">
                                                <v-icon color="#707070" size="15">mdi-close</v-icon>
                                            </v-btn>
                                        </v-card-title>
                                        <v-card-text class="pt-0">
                                                    ...
                                                    <v-col cols="12" md="12" class="pt-0">
                                                        <v-container fluid class="pt-0">
                                                            <v-row>
                                                                ...
                                                                <v-col cols="12" xs="12" sm="12" md="12" class="text-right py-0">
                                                                    <Secondary-Button id="AutoTest_MP66" class="btn-122x36 mr-4" title="Remove" @click.native="DeleteProject(2)"></Secondary-Button>
                                                                    <Default-Button-Outlined id="AutoTest_MP67" class="btn-122x36" title="Cancel" @click.native="RemoveProjectDailog=false"></Default-Button-Outlined>
                                                                </v-col>
                                                            </v-row>
                                                        </v-container>
                                                    </v-col>
            ...
            <v-dialog v-model="IsActiveToggleDialog" width="800">
                <v-card class="elevation-0">
                    <v-card-title primary-title class="page-head py-4">
                        ...
                        Change State
                        <v-spacer></v-spacer>
                        <v-btn id="AutoTest_MP69" text @click="IsActiveToggleDialog=false">
                            <v-icon color="#707070" size="15">mdi-close</v-icon>
                        </v-btn>
                    </v-card-title>
                    <v-card-text class="pt-0">
                                ...
                                <v-col cols="12" md="12" class="pt-0">
                                    <v-container fluid class="pt-0">
                                        <v-row>
                                            ...
                                            <v-col cols="12" xs="12" sm="12" md="12" class="text-right py-0">
                                                <v-btn id="AutoTest_MP70" color="secondary elevation-0" class="mr-2" @click="ChangeState()">Change</v-btn>
                                                <v-btn id="AutoTest_MP71" outlined color="grey" @click="IsActiveToggleDialog=false, GetProjectListByInitID()">Cancel</v-btn>
                                            </v-col>
            ...
            <v-dialog v-model="dlgSetasTemplate" width="800" persistent>
                <v-card>
                    <v-card-title primary-title="" class="page-head pb-0">
                        ...
                        Save as Template
                        <v-spacer></v-spacer>
                        <v-btn id="AutoTest_MP72" text="" @click="dlgSetasTemplate = false">
                            <v-icon color="#707070" size="15">mdi-close</v-icon>
                        </v-btn>
                    </v-card-title>
                    <v-card-text>
                    ...
                    <v-card-actions class="pb-4">
                        <v-col>
                            <Outlined-Button-Dark id="AutoTest_MP73" class="btn-122x36 float-right" title="Cancel" @click.native="dlgSetasTemplate=false"></Outlined-Button-Dark>
                            <Primary-Button-Light id="AutoTest_MP74" class="btn-122x36 float-right mr-4" title="Confirm" @click.native="dlgSetasTemplate=false,InsertProjectTemplate() " v-show="AddProjectTempSteps==2"></Primary-Button-Light>
                            <v-btn id="AutoTest_MP75" outlined color="grey" class="btn-122x36 float-right" @click="AddProjectTempSteps=1" v-show="AddProjectTempSteps==2">Previous</v-btn>
                            <Primary-Button-Light id="AutoTest_MP76" class="btn-122x36 float-right mr-4" title="Next" @click.native="AddProjectTempSteps=2" v-show="AddProjectTempSteps==1" v-bind:disabled="Template.length == 0 || Template.trim() == ''"></Primary-Button-Light>
                        </v-col>
                    </v-card-actions>
                </v-card>
            </v-dialog>
...
<script type="text/javascript">
    ...
    export default {
        ...
        mounted: async function () {
            ...
            this.PreLoader = true;
            this.InitiativeID = 1; //this.$route.query.InitID;
            await importscript();
