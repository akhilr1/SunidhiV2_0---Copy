    ...
    <div>
        ...
        <v-container fluid class="px-0">
            <v-row>
                ...
                <v-spacer></v-spacer>
                <v-col md="auto">
                    <v-btn depressed height="39" class="mx-2" :to="{name: 'OutcomeReportTable', query: {ProjID: $route.query.ProjID } }" color="green" dark>Report</v-btn>
                </v-col>
                <v-col v-if="ActivityList.length>0" md="auto">
                    <!--Create Menu-->
                    <v-menu offset-y transition="scroll-y-transition" v-bind:disabled="!screenAccess">
                        <template v-slot:activator="{ on }">
                            <v-btn v-on="on" class="success-btn mr-4 float-left" depressed dark height="39" @click="fnCreateOutcome()">
                                Create
            ...
            <v-row>
                ...
                <v-col v-if="ActivityList.length==0" class="text-center">
                    ...
                    <p class="heading-3-dark font-weight-medium pt-6">No Indicators data available</p>
                    <v-btn id="AutoTest_IP205" @click="fnCreateOutcome(),SubActivityTotalWeightage =0" class="success-btn mt-2" depressed dark height="37" v-bind:disabled="!screenAccess">
                        Create Indicators
                    </v-btn>
                </v-col>
            ...
            <!--Tree table section-->
            <v-row>
                <v-col v-if="filterActivityList.length >0">
                        ...
                        <template #headerTemplate="headerProps"
                            ...
                            <div v-bind="headerProps">
                                <div class="row header" style="background-color:#F5F5F5;max-height:50px">
                                    <div class="cell pt-0" style="max-width:20px">
                                        <v-checkbox color="#52B962" v-model="Commoncheck" @change="CheckFn()"></v-checkbox>
                                    </div>

                                    <div class="cell" style="max-width:55px"></div>

                                    <table style="font-size:13px;color: #707070;" class="cell mt-2 mb-7">
                                        <tr>
                                            <th style="flex-grow:1;" :style="[isMobile ? {'min-width' : '50px'} : {'min-width' : '180px'} ]"><b>{{ActivityListHeader[0].label}}</b></th>
                                            <th style="width:175px"><b>{{ActivityListHeader[1].label}}</b></th>
                                            <th style="width:175px"><b>{{ActivityListHeader[2].label}}</b></th>
                                            <th style="width:175px"><b>{{ActivityListHeader[3].label}}</b></th>
                                            <!--<th style="width:130px"><b>{{ActivityListHeader[4].label}}</b></th>-->
                                            <th style="width:175px"><b>{{ActivityListHeader[5].label}}</b></th>
                                            <th style="width:175px"><b>{{ActivityListHeader[6].label}}</b></th>
                                            <th style="width:70px"><b>{{ActivityListHeader[7].label}}</b></th>
                                        </tr>
                                    </table>
                        ...
                        <template #leafTemplate="leafProps">
                            <div v-bind="leafProps">
                                <div class="row" style="max-height:55px;">
                                    ...
                                    <div class="cell text-limit pt-5" style="flex-grow:1">
                                        ...
                                        <!--v-if="$route.name == 'MyprojectsDeployedIndividualKPIOutcomes'"-->  <!--commented on 08-0-202-->
                                        <router-link :to="{name:'MyProjectDeployedIndividualOutcomeActivityLog',query: { WsID: WorkspaceID , Access:RWAccess, OutcomeId: leafProps.rowData.ActivityId,ProjID: projectInfo.ProjectId, InitID: initiativeInfo.InitiativeId, Type: 'Edit' }}">
                                            <v-avatar color="white" size="20" class="elevation-1 mr-2">
                                                <v-icon size="15" color="primary">mdi-chart-areaspline</v-icon>
                                            </v-avatar>
                                            {{ leafProps.rowData.ActivityName }}
                                    ...
                                    <div class="cell pt-5" style="max-width: 180px">
                                    ...
                                    <div class="cell" style="max-width: 180px">
                                    ...
                                    <div class="cell pt-4" style="max-width: 70px">
                                        <v-menu close-on-content-click="" transition="scroll-y-transition" left="" offset-y="" v-bind:disabled="!screenAccess">
                                            ...
                                            <v-list dense="" color="white" light="" width="auto" class="action-button-list">
                                                <v-list-item id="AutoTest_IP215" link="" @click.stop="fnInitializeAddEditActivity(leafProps.rowData),fnEditOutcomeDlg(leafProps.rowData)">
                                                    <v-list-item-icon>
                                                ...
                                                <!--<v-list-item id="AutoTest_IP218" link="" @click.stop="fnAddSubActivityInSubActivity(leafProps.rowData.ActivityId,leafProps.rowData)">
                                                    <v-list-item-icon>
                                                        <v-icon>
            ...
            <create-outcome-dialog v-if="createActivityDlg"
                                   ...
                                   @unloadDialog="fnUnloadOutcomeDlg()"></create-outcome-dialog>
            <!--Assign User Dialog-->
                                    ...
                                    <v-container fluid class="pt-0">
                                        <v-row>
                                            ...
                                            <v-col v-else cols="12" md="12" class="pt-0">
                                                <h4 class="text-center heading-3">Are you sure you want to remove below outcomes ?</h4>
                                            </v-col>
...
<script type="text/javascript">
    ...
    import CreateOutcomeDialog from "../Projects/ProjectComponents/CreateOutcomeDialog.vue";
    ...
    async function importscript() {
        return Promise.all([
            import("../../BL/Outcomes.js").then((mod) => {
                objManageProjectActivity = new mod.Outcomes();
            }),
        ]);
    }
    ...
    export default {
        ...
        computed: {
            ...
            computedHeaders() {
                ...
                if (this.AddEditActivity.StartDate && this.AddEditActivity.EndDate) {
                    if (this.headers_edit && this.headers_edit.length) {
                        ...
                        let sdate = new Date(this.AddEditActivity.StartDate)
                        let edate = new Date(this.AddEditActivity.EndDate)
                        sdate.setDate(1)
                        do {
                            this.headers.push({
        ...
        methods: {
            ...
            async fnCreateOutcome(item, mode) {
            ...
            // async fnGetOutcomeActualValue(outcomeId) {
            //     var result = await objManageProjectActivity.GetOutcomeActualValue(outcomeId);
            //     if (result.status == "Unhandled Exception") {
            //         this.$router.push({ name: "error" });
            //     } else if (result.data) {
            //         this.actualValue = result.data;

            //     } else {
            //         this.Msg = "Failed to get actual values " ;
            //         this.errorSnackbarMsg = true;
            //     }
            // },
            ...
            async fnGetProjectActivityList() {
                var result = await objManageProjectActivity.FetchAllOutcomeList(this.projectInfo.ProjectId);
                if (result.status == "Unhandled Exception") {
                    this.$router.push({ name: "error" });
                } else if (result.data) {
                    ...
                    this.ParentActivityList = result.data;
                } else {
                    this.Msg = "Failed to get outcome list of Project " + this.projectInfo.ProjectName;
                    this.errorSnackbarMsg = true;
                }
                this.PreLoader = false
            },
            ...
            async fnInitializeAddEditActivity(item) {
                ...
                if (item != null) {
                    ...
                    //this.fnGetOutcomeActualValue(item.ActivityId)
                    this.activityTarget = item.SurveyTarget
                    this.headers_edit = item.lstProjectOutcomeMonthwiseData
                    if (this.headers_edit && this.headers_edit.length) {
                    ...
                    if (item.OutcomeQuantitativeLogic != null && item.OutcomeQuantitativeLogic != undefined && item.OutcomeQuantitativeLogic.length > 0) {
                        for (var i = 0; i < item.OutcomeQuantitativeLogic.length; i++) {
                            const obj = new Object();
                            obj.QuestionId = item.OutcomeQuantitativeLogic[i].QuestionId;
                            obj.SectionId = item.OutcomeQuantitativeLogic[i].SectionId;
                            obj.SurveyId = item.OutcomeQuantitativeLogic[i].SurveyId;
                            obj.QuestionName = item.OutcomeQuantitativeLogic[i].QuestionName;
                            obj.SectionName = item.OutcomeQuantitativeLogic[i].SectionName;
                            obj.SurveyName = item.OutcomeQuantitativeLogic[i].SurveyName;
                            obj.BeneficiaryTypeQstnID = item.OutcomeQuantitativeLogic[i].BeneficiaryTypeQstnID;
                            obj.SurveyPrimeLogic = [];

                            if (item.OutcomeQuantitativeLogic[i].lstSurveyPrimeLogic != null && item.OutcomeQuantitativeLogic[i].lstSurveyPrimeLogic != undefined && item.OutcomeQuantitativeLogic[i].lstSurveyPrimeLogic.length > 0) {
                                for (var k = 0; k < item.OutcomeQuantitativeLogic[i].lstSurveyPrimeLogic.length; k++) {
                                    const obj1 = new Object();

                                    obj1.SurveyLogicPrimeID = item.OutcomeQuantitativeLogic[i].lstSurveyPrimeLogic[k].SurveyLogicPrime;
                                    obj1.SurveyLogicPrime = item.OutcomeQuantitativeLogic[i].lstSurveyPrimeLogic[k].SurveyLogicPrimeType;
                                    obj1.SurveyLogicPrimeAnswer = item.OutcomeQuantitativeLogic[i].lstSurveyPrimeLogic[k].SurveyLogicPrimeAnswer;
                                    obj1.SurveyLogicCondition = item.OutcomeQuantitativeLogic[i].lstSurveyPrimeLogic[k].SurveyLogicCondition;
                                    obj1.id = k + 1;
            ...
            async fnAddActivity(type) {
                ...
                if (this.$refs.Form.validate() === true) {
                    ...
                    data.append("OutcomeName", item.ActivityName);
                    ...
                    data.append("DatacollectionType", item.ActivityType);
                    ...
                    if (item.ActivityType == "Quantitative") {
                        ...
                        data.append("SurveyTarget", this.activityTarget);
                        ...
                        if (this.AddEditActivity.OutPutActivityUsers != null && this.AddEditActivity.OutPutActivityUsers.length > 0) {
                            for (var i = 0; i < this.AddEditActivity.OutPutActivityUsers.length; i++) {
                                data.append('OutcomeQuantitativeLogic[' + i + '].QuestionId', this.AddEditActivity.OutPutActivityUsers[i].QuestionId);
                                data.append('OutcomeQuantitativeLogic[' + i + '].SectionId', this.AddEditActivity.OutPutActivityUsers[i].SectionId);
                                data.append('OutcomeQuantitativeLogic[' + i + '].SurveyId', this.AddEditActivity.OutPutActivityUsers[i].SurveyId);
                                data.append('OutcomeQuantitativeLogic[' + i + '].BeneficiaryTypeQstnID', this.AddEditActivity.OutPutActivityUsers[i].BeneficiaryTypeQstnID);
                                ...
                                if (this.AddEditActivity.OutPutActivityUsers[i].SurveyPrimeLogic != null && this.AddEditActivity.OutPutActivityUsers[i].SurveyPrimeLogic != undefined && this.AddEditActivity.OutPutActivityUsers[i].SurveyPrimeLogic.length > 0) {
                                    for (var k = 0; k < this.AddEditActivity.OutPutActivityUsers[i].SurveyPrimeLogic.length; k++) {
                                        data.append('OutcomeQuantitativeLogic[' + i + '].lstSurveyPrimeLogic[' + k + '].SurveyLogicPrime', this.AddEditActivity.OutPutActivityUsers[i].SurveyPrimeLogic[k].SurveyLogicPrimeID);
                                        data.append('OutcomeQuantitativeLogic[' + i + '].lstSurveyPrimeLogic[' + k + '].SurveyLogicPrimeAnswer', this.AddEditActivity.OutPutActivityUsers[i].SurveyPrimeLogic[k].SurveyLogicPrimeAnswer);
                                        data.append('OutcomeQuantitativeLogic[' + i + '].lstSurveyPrimeLogic[' + k + '].SurveyLogicCondition', this.AddEditActivity.OutPutActivityUsers[i].SurveyPrimeLogic[k].SurveyLogicCondition);
                                    }
                                }
                            }
                            this.OutputErrorMessage = "";
                    ...
                    if (this.headers && this.headers.length && this.monthlyTarget) {
                        for (var x = 0; x < this.headers.length; x++) {
                            ...
                            data.append('lstProjectOutcomeMonthwiseData[' + x + '].Budget', this.headers[x].budget);
                    ...
                    var result = await objManageProjectActivity.AddOutcome(data);
                    ...
                    if (result.status == "Unhandled Exception") {
                        ...
                        if (result.status == "success") {
                            if (type == "AddSubActivity") {
                                ...
                                this.Msg = "Outcome Successfully updated";
                                this.successSnackbarMsg = true;
                                this.fnInitializeAddEditActivity(null);
                                await this.fnGetProjectActivityList();
                            } else {
                                this.Msg = "Outcome Successfully added";
                                this.successSnackbarMsg = true;
                                this.fnInitializeAddEditActivity(null);
                                await this.fnGetProjectActivityList();
                                this.$refs.Form.resetValidation()
            ...
            async fnRemoveActivity() {
                ...
                if (this.RemoveActivityItem) {
                    data.append("OutcomeIdList", this.RemoveActivityItem.ActivityId);
                } else {
                    ...
                    data.append("OutcomeIdList", actids);
                }
                var result = await objManageProjectActivity.RemoveOutcome(data);
                if (result.status == "Unhandled Exception") {
                    this.PreLoader = false;
                    this.$router.push({ name: "error" });
                } else if (result.status == "success") {
                    ...
                    this.Preloader = false;
                    this.Msg = "Failed to remove an Outcomes.";
                    this.errorSnackbarMsg = true;
                }
            },
            ...
            async fnAssignUser() {
                ...
                var data = new FormData();
                data.append("OutcomeId", this.AssignUser.ActivityId);
                data.append("OutcomeUsersList", JSON.stringify(this.AssignUser.selectedProjectUserList));
                var result = await objManageProjectActivity.AssignUserToOutcome(data);
                if (result.status == "Unhandled Exception") {
                    this.$router.push({ name: "error" });
                } else if (result.status == "success") {
            ...
            async fnCreateOuput() {
                ...
                this.SubActivityTotalWeightage = 0;
                this.AddEditActivityDialog = true;
                this.AddEditActivityDialogHeader = 'Create Outcome';
                this.AddEditActivity.OutPutActivityUsers = [];
                this.OutputErrorMessage = "";
                this.SurveyErrorMessage = "";
            },
