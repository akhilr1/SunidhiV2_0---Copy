        ...
        <v-card-title style="height:60px" class="pt-4">
            <h5 class="text-left page-head">
                <v-avatar color="white" size="20" class="elevation-1 mr-2 mb-1">
                    <v-icon size="15" color="primary">mdi-file-document-box-search-outline</v-icon>
                </v-avatar>
                Search Venture
            </h5>
            <v-spacer></v-spacer>
            ...
            <v-menu offset-y transition="scroll-y-transition" v-bind:disabled ="!screenAccess">
                <template v-slot:activator="{ on }">
                    <v-btn height="39" v-on="on"  outlined class="elevation-0 mr-4 outlined-btn-dark">
                        Bulk Actions
                        <v-icon>
                            mdi-chevron-down
                        </v-icon>
                ...
                <v-list color="white" light="" width="auto" max-width="250" class="action-button-list">
                    ...
                    <v-list-item id="AutoTest_V55" @click.stop="fnBulkWhishlistClick()">
                        <v-list-item-icon>
                    ...
                    <!-- Wishlist dailog-->
                    <v-dialog v-model="dlgBulkWhishlist" width="800">
                        <v-card>
                            <v-card-title primary-title class="page-head py-4">
                                ...
                                Whishlist
                                <v-spacer></v-spacer>
                                <v-btn id="AutoTest_V56" text="" @click="dlgBulkWhishlist=false">
                                    ...
                                    <v-row no-gutters>
                                        <v-col cols="12" md="12" class="pt-0">
                                            <v-container fluid class="pt-0">
                                                <v-row>
                                                    <v-col cols="12" md="12" class="pt-0">
                                                        <h4 class="text-center heading-3">Are you sure want to wishlist this Project(s) ?</h4>
                                                    </v-col>

                                                    <v-col cols="12" xs="12" sm="12" md="12" class="text-right py-0">
                                                      <Secondary-Button id="AutoTest_V57" class="btn-122x36 mr-4" title="Confirm" @click.native="AddToWishlist(1),dlgBulkWhishlist = false"></Secondary-Button>
                                                      <Default-Button-Outlined id="AutoTest_V58" class="btn-122x36" title="Cancel" @click.native="dlgBulkWhishlist=false"></Default-Button-Outlined>
                                                    </v-col>
                                                </v-row>
                                            </v-container>
                                        </v-col>
            ...
            <v-menu :close-on-content-click="false" offset-y transition="scroll-y-transition" v-model="filterMenu">
                <template v-slot:activator="{ on }">
                    <v-btn id="AutoTest_V59" v-on="on" height="39" outlined dense class="outlined-btn-dark mr-2" @click="">
                ...
                <v-list color="white" light="" width="auto" max-width="250" min-width="50" class="action-button-list pa-2">
                    ...
                    <div class="d-inline-flex">
                        <v-btn id="AutoTest_V60" height="32" width="35" block color="success-btn" dark class="elevation-0 mr-1" @click="fnClickFilterDone()">
                            Apply
                        </v-btn>
                        <v-btn id="AutoTest_V61" height="32" width="35" block color="grey" outlined="" class="elevation-0" @click="fnResetFilter()">
                            Reset
                        </v-btn>
                    </div>
                ...
                <v-data-table :headers="ProjectTableheaders"
                    ...
                    <template v-slot:item.ProjectName="{item}">
                        <v-list class="py-0 table-v-list">
                            <v-list-item class="my-0 px-0" link :to="{ name: 'ManageVentureDetails', query: { WsID: WorkspaceID  , Access: RWAccess,InitID : item.InitiativeId, ProjID : item.ProjectId, VentureType :'Search Venture',FundUtilized : item.FundUtilized} }">
                    ...
                    <template v-slot:item.Actions="{item}">
                        <v-menu close-on-content-click offset-y left transition="scroll-y-transition" v-bind:disabled="!screenAccess">
                            ...
                            <v-list color="white" light width="auto" class="action-button-list">
                                <v-list-item id="AutoTest_V62" @click.stop="GetProjectID(item.ProjectId,item.ProjectName,item.Budget,item.FundedAmount),dlgCommitFund = true">
                                    <v-list-item-icon>
                                ...
                                <v-list-item id="AutoTest_V63" link @click.stop="GetProjectID(item.ProjectId,item.ProjectName),dlgWhishlist = true,ProjectName=item.ProjectName">
                                    <v-list-item-icon>
                                        <v-icon>mdi-star-outline</v-icon>
                                    </v-list-item-icon>
                                    <v-list-item-content>
                ...
                <v-dialog v-model="dlgCommitFund" width="800">
                    <v-card>
                        <v-card-title primary-title class="page-head py-4">
                            ...
                            Commit Fund
                            <v-spacer></v-spacer>
                            <v-btn id="AutoTest_V64" text="" @click="dlgCommitFund=false,CommitedFund=''">
                                <v-icon color="#707070" size="15">mdi-close</v-icon>
                            </v-btn>
                        </v-card-title>
                        <v-card-text class="pt-0">
                                    ...
                                    <v-col cols="12" md="12" class="pt-0">
                                        <v-container fluid class="pt-0">
                                            <v-row>
                                                ...
                                                <v-col cols="12" xs="12" sm="12" md="12" class="text-right py-0">
                                                    <Secondary-Button id="AutoTest_V65" class="btn-122x36 mr-4" title="Confirm" @click.native="InsertCommitFund('Funded Project'),dlgCommitFund = false" v-bind:disabled="CommitedFund <= 0" @click=""></Secondary-Button>
                                                    <Default-Button-Outlined id="AutoTest_V66" class="btn-122x36" title="Cancel" @click.native="dlgCommitFund=false,CommitedFund=''"></Default-Button-Outlined>
                                                </v-col>
                                            </v-row>
                                        </v-container>
                                    </v-col>
                ...
                <v-dialog v-model="dlgWhishlist" width="800">
                    <v-card>
                        <v-card-title primary-title class="page-head py-4">
                            ...
                            Whishlist
                            <v-spacer></v-spacer>
                            <v-btn id="AutoTest_V67" text="" @click="dlgWhishlist=false">
                                <v-icon color="#707070" size="15">mdi-close</v-icon>
                            </v-btn>
                        </v-card-title>
                        <v-card-text class="pt-0">
                                    ...
                                    <v-col cols="12" md="12" class="pt-0">
                                        <v-container fluid class="pt-0">
                                            <v-row>
                                                ...
                                                <v-col>
                                                    <h4 class="text-center heading-3">
                                                        <v-avatar class="mr-2 elevation-1" size="40" color="#fff">
                                                            <v-icon color="#707070" size="25">mdi-file-document-outline</v-icon>
                                                        </v-avatar>
                                                        {{ProjectName}}
                                                    </h4>
                                                </v-col>
                                                ...
                                                <v-col cols="12" xs="12" sm="12" md="12" class="text-right py-0">
                                                    <Secondary-Button id="AutoTest_V68" class="btn-122x36 mr-4" title="Confirm" @click.native="AddToWishlist(2),dlgWhishlist = false"></Secondary-Button>
                                                    <Default-Button-Outlined id="AutoTest_V69" class="btn-122x36" title="Cancel" @click.native="dlgWhishlist=false"></Default-Button-Outlined>
                                                </v-col>
                                            </v-row>
                                        </v-container>
                                    </v-col>
...
<script type="text/javascript">
    ...
    export default {
        ...
        methods: {
            ...
            /// <summary>
            /// GetProjectListOfVenture :- Function used to fetch Project List of Venture based on  WorkspaceID.
            /// </summary>
            /// <param name=""></param>
            /// <returns> </returns>
            async GetProjectListOfVenture() {
                ...
                var result = await objVenture.GetProjectListOfVenture(data);
            ...
            /// <summary>
            /// AddToWishlist :- Function used to Add Projects to WishList
            /// </summary>
            ...
            /// <summary>
            /// fnBulkWhishlistClick :- Function used to Add Projects to WishList Bulk.
            /// </summary>
            /// <param name=""></param>
            /// <returns> </returns>
            fnBulkWhishlistClick() {
                if (this.selectedProjectID.length > 0) {
                    this.dlgBulkWhishlist = true;
                } else {
                    this.Msg = "Please select atleast one project";
